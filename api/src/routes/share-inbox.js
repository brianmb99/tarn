...
async function checkInboxFetchRateLimit(env, request) {
  ...
  const { allowed, count } = await checkAndIncrementRateLimit(env.RATE_KV, key, MAX_INBOX_FETCHES_PER_HOUR);
  return { allowed, remaining: Math.max(0, MAX_INBOX_FETCHES_PER_HOUR - count) };
}
