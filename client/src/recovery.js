...
// except `sendRecoveryKitEmail` which posts the rendered PDF bytes to the
// Tarn API for transparent forwarding to an email relay (the API holds the
// PDF in memory only for the duration of the forward request — see
// docs/TARN_PROTOCOL.md § Recovery email forwarder).
 ...
 * passing to {@link sendRecoveryKitEmail}.
 *
...
export function renderRecoveryPDF({ phrase, appName, generatedAt }) {
  ...
  const lines = [
    ...
    { y: 200, font: 'F1', size: 13, text: 'If you received this by email' },
    { y: 178, font: 'F2', size: 11, text: 'Save the attachment somewhere safe and DELETE THE EMAIL. Email' },
    { y: 164, font: 'F2', size: 11, text: 'inboxes are a common attack target — your phrase should not stay there.' },

    { y: 110, font: 'F2', size: 9, text: 'Tarn never stores your recovery phrase. This kit was generated by your' },
    { y: 98, font: 'F2', size: 9, text: 'device and (if you opted in) forwarded through Tarn\'s relay without persistence.' },
  ];
