...
- **`tarn.accountKey`** — account-key lifecycle: `view({ password })` (step-up-gated retrieval of the stored key for Model B accounts, with a client-side wrap-pinning check), `enableKeyStorage` / `disableKeyStorage` (Model A↔B toggle), `rotate({ password })` (new account key, full DEK-chain re-wrap), `isStored()`. The SDK deliberately does NOT render recovery kits — it returns the 24-word account-key string and nothing else; apps build their own kit (PDF, printable HTML, clipboard) and own delivery. The SDK never persists the decrypted account key and exposes no transport surface for it. Account recovery itself is on the top-level client (`tarn.recoverAccount({ phrase, newUsername, newPassword })`) since it's pre-auth.
...
client/src/
  ...
  recovery.ts            — BIP39 phrase generation, recovery KEK derivation (no kit rendering — apps own that)
