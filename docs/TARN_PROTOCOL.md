...
**App-wide constant PRF salt (tarn#59).** `prf_salt` is a single, fixed, app-wide constant — the same 32-byte value for every Tarn passkey, at both register time and authentication time. It is `SHA-256("tarn-passkey-prf-constant-salt-v1")`, derived server-side and never persisted as a secret (it is, by design, returned in the clear). Both `POST /auth/passkey/register-options` and `POST /auth/passkey/authentication-options` return this same constant in `options.extensions.prf.eval.first`.

The constant salt does **not** weaken per-credential isolation: the WebAuthn PRF extension is HMAC keyed by a per-**credential** secret the authenticator generates at registration and never exports — the RP-supplied salt is only the HMAC *message*. So `PRF(constant)` is still unique per credential: two passkeys fed the same salt produce different 32-byte outputs, hence different `passkey_KEK`s, hence per-credential-unique AES-KW wrappings.

**Why the salt is a constant and not per-account.** The usernameless / discoverable sign-in flow (`POST /auth/passkey/authentication-options` with no `credential_id`) does not know which account is signing in — that is the point of resident-credential login. Returning a per-account (or per-credential) salt would require first resolving the account, i.e. reading the credential table on an unauthenticated public endpoint — exactly the enumeration tarn#59 removed. A value the server can return *without* knowing the account must be account-independent; a single app-wide constant is the only shape that satisfies that while keeping `PRF(salt)` unique per credential. The earlier scheme returned a `credential_id → prf_salt` map (`options.extensions.prf.evalByCredential`) on every public auth-options call; **`evalByCredential` is deprecated** and no longer emitted.
...
**Migration (one-time re-registration).** Passkeys registered *before* the tarn#59 constant-salt switch were wrapped under a per-credential *random* salt, so `PRF(constant)` will not reproduce their wrapping key and their `passkey_prf` wrappings will not unwrap. Such a passkey is invalidated *for unwrapping only* until re-registered; the account is never locked out because the **password and recovery factors are independent wrappings, unaffected by this change**. The SDK surfaces a clean `StalePasskeyError` ("re-register your passkey") instead of crashing. Operator recovery: log in with password → remove the old passkey → add a passkey (now wrapped under `PRF(constant)`) → passkey login works.
...
CREATE TABLE passkey_credentials (
  ...
  prf_salt     TEXT NOT NULL,            -- VESTIGIAL (tarn#59): now stores the app-wide constant salt for every row; no longer read at auth time. Kept NOT NULL to avoid a migration / Arweave-mirror churn; may be dropped in a future migration.
...
- `POST /api/v1/auth/passkey/register-options` — JWT-authed (logged-in user). Returns `{ options, prf_salt }`, where `prf_salt` (and `options.extensions.prf.eval.first`) is the app-wide constant salt (tarn#59) — the same value the authentication-options path returns, so a passkey is wrapped under `PRF(constant)` at register and unwrapped under `PRF(constant)` at sign-in without any per-credential salt lookup. Server stores the registration challenge in `webauthn_challenges` (60 s TTL, single-use).
...
- `POST /api/v1/auth/passkey/authentication-options` — public, **per-IP rate-limited** (60/hr; tarn#59). Body: `{ credential_id? }`. Returns `{ options, rp_id }`. The single app-wide constant PRF salt rides in `options.extensions.prf.eval.first` (see [PRF derivation](#prf-derivation)); there is **no** per-credential `evalByCredential` map (deprecated, tarn#59 full close) and **no** top-level `allow_credentials` dump. **The endpoint issues no `SELECT` against `passkey_credentials` in either flow** — the per-credential enumeration is fully eliminated. Two flows: with `credential_id` (account-identified re-tap) the server narrows `options.allowCredentials` to that one credential (the supplied ID is trusted directly and re-verified at `/authenticate`); without it (usernameless / discoverable sign-in) `options.allowCredentials` is empty and the authenticator self-presents its resident credentials — the standard discoverable WebAuthn shape. Server stores the authentication challenge in `webauthn_challenges` (60 s TTL).
...
```json
{
  ...
  "prf_salt": "<base64url 32-byte salt — VESTIGIAL post-tarn#59: the app-wide constant; carried for blob-shape stability, not read on rebuild>",
