...
User-facing tables rebuildable from Arweave. Entries self-heal on cache miss. Accounts rebuilt from `Type=cred` blobs (with `RLk` secondary tag for recovery-factor discovery, since 2026-05). Apps rebuilt from `Type=app-reg` blobs (Phase A of the Arweave-recoverability fix). Passkey credentials rebuilt from `Type=passkey-reg` blobs (Phase B; see "Arweave mirror — `Type=passkey-reg` blobs" under Passkey factor (Phase 6)). Transient state (sessions, nonces, step-up tokens, WebAuthn challenges, rate-limit counters, idempotency cache) is acceptable loss on rebuild.

The operational rebuild tool (Phase C, `tools/rebuild-from-arweave.mjs`) walks Arweave and reconstructs D1 in dependency order. See `docs/ARWEAVE_RECOVERABILITY_FIX_PLAN.md` for the cross-phase plan.
