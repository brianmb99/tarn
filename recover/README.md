...
Tool details: `dist/forever.html` is built deterministically (same input bytes → same output). The publish script is dry-run by default; `--confirm` enables real publish, and `--app-id <id>` is required (it stamps the `App-Id` tag that keeps each app's pointer chain separate). Each publish writes the page (`Type=forever-page`) plus a tiny `Type=forever-page-pointer` blob whose body is the just-published page txid — Arweave-native "latest" discovery without needing a Tarn-side endpoint. Discovery queries are **owner-pinned** (filtered to the operator wallet's address, which the script prints) so tag-squatters can't hijack "latest". See [`examples/forever/README.md` §"Arweave publish workflow"](examples/forever/README.md#arweave-publish-workflow) for the full flow (build, dry-run, confirm, tag scheme, idempotency, discovery query, sharing the txid with users).

### The bootstrap page (the permanent recovery URL)

Because every publish gets a new txid, a bookmark to one page version goes stale as the page improves. The [forever-bootstrap](examples/bootstrap/README.md) solves this: a tiny dependency-free page, built per app (`scripts/build-bootstrap.mjs`) and published once (`publish-forever.mjs --bootstrap`), that runs the owner-pinned pointer query, verifies the result, and forwards the user to the latest page — with a version list and a baked-in static fallback if every gateway lookup fails. Its txid is the one permanent URL an app puts in recovery kits (headline link), alongside the concrete page txid current at kit time (pinned fallback). The bootstrap takes no credentials and never needs republishing for SDK releases.
