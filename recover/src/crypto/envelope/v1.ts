/**
 * Envelope v1 decoder — the multi-factor `dek_chain` shape Tarn writes today.
 *
 * Borrowed near-verbatim from `client/src/crypto.ts` lines 812-1075. This
 * module is the **only** place in `@tarn/recover` that knows about the v1
 * wire shape; everything else routes through the version-dispatch in
 * `./index.ts`.
 *
 * v1 wire shape (JSON):
 *   {
 *     "v": 1,
 *     "kdf": "argon2id",
 *     "kdf_params": { m_kib, t, p },
 *     "recovery": { "kdf": "argon2id", "kdf_params": {...}, "salt": "<base64>" },
 *     "dek_chain": [
 *       { "gen": <int>, "wrappings": [
 *           { "factor": "password",        "wrapped": "<base64>" },
 *           { "factor": "recovery_phrase", "wrapped": "<base64>" },
 *           { "factor": "passkey_prf",     "wrapped": "<base64>", "credential_id": "<id>" },
 *         ] },
 *       ...
 *     ]
 *   }
 *
 * **Forward-compat contract:** This file is frozen as the v1 reader. Future
 * versions live in sibling modules (`v2.ts`, ...) registered in `./index.ts`.
 * Bug fixes that don't change wire interpretation are allowed; semantic
 * changes are not — they must ship as a new version. See
 * `recover/fixtures/README.md` for the contract.
 */
...
export type EnvelopeV1Version = 1;
export const ENVELOPE_V1_VERSION: EnvelopeV1Version = 1;

export type ChainWrapping = {
  factor: string; // narrowed to `Factor` by the wrap helpers
  wrappedBase64: string;
  /** Populated for `passkey_prf` wrappings only. */
  credentialId?: string;
};
...
export type ParsedEnvelopeV1 = {
  envelopeVersion: EnvelopeV1Version;
...
/**
 * Parse a v1 envelope from its already-JSON-parsed object form.
 *
 * Caller is `./index.ts`'s dispatch — it has already verified `parsed.v === 1`
 * before handing the object off here. `parsed.kdf` is also pre-checked, but
 * we re-validate locally so this module is safe to invoke directly.
 *
 * Throws on any malformed-shape detection. Does not unwrap any keys; that's
 * `unwrapEnvelopeV1`.
 */
export function parseEnvelopeV1(parsed: Record<string, unknown>): ParsedEnvelopeV1 {
...
/** Result of unwrapping every DEK in a v1 chain. */
export type UnwrappedEnvelopeV1 = {
...
/**
 * Unwrap every DEK in a v1 chain via the named factor's KEK.
 *
 ...
 */
export async function unwrapEnvelopeV1(
  parsed: ParsedEnvelopeV1,
  unwrappingKey: CryptoKey,
  factor: string = FACTOR_PASSWORD,
  credentialId?: string,
): Promise<UnwrappedEnvelopeV1> {
